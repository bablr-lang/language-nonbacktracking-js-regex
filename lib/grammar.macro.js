...
export const grammar = class RegexGrammar {
  ...
  *CharacterClassElement() {
    yield eat(
      m`<Any />`,
      e([m`.: <CharacterSet /\\[dswp]/i />`, m`.: <Gap '\\g' />`, m`.+: <*Character />`]),
    );
  ...
  *EscapeSequence({ state, ctx, value: props }) {
    ...
    if ((m_ = yield match(re`/[\\/nrt0]/`))) {
      const match_ = ctx.sourceTextFor(m);
      yield eat(m`code: <*Keyword ${buildString(match_)} closeSpan='Escape' />`);
    } else if ((m_ = yield match(getSpecialPattern(parentSpan, ctx.reifyExpression(props))))) {
      const match_ = ctx.sourceTextFor(m);
      yield eat(m`code: <*Keyword ${buildString(match_)} closeSpan='Escape' />`);
