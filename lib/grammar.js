 ...
 * @generatedfrom ./grammar.macro.js#b135151b0391d4e43886c65a5728928bfadae205
...
import { e, eat, eatMatch, match, holdForMatch, guard, bindAttribute, fail } from '@bablr/helpers/grammar';
...
export const grammar = class RegexGrammar {
  ...
  *Element() {
    ...
    yield eat(m`.+: <Any />`, e([m`<CharacterClass '[' />`, m`<Group '(?:' />`, m`<Assertion /[$^]|\\b/i />`, m`<Gap '\\g' />`, m`<CharacterSet /\.|\\[dswp]/i />`, m`<*Character />`]));
    if (yield match(re`/[*+?{]/`)) {
  ...
  *Assertion() {
    yield eat(m`<Any />`, e([m`<*StartOfInputAssertion '^' />`, m`<*EndOfInputAssertion '$' />`, m`<*WordBoundaryAssertion /\\b/i />`]));
  }
  ...
  *CharacterClassElement() {
    yield eat(m`<Any />`, e([m`.: <CharacterSet /\\[dswp]/i />`, m`.: <Gap '\\g' />`, m`.+: <*Character />`]));
    if (yield match('-')) {
  ...
  *CharacterSet() {
    yield eat(m`.+: <Any />`, e([m`<AnyCharacterSet '.' />`, m`<DigitCharacterSet /\\[dD]/  />`, m`<SpaceCharacterSet /\\[sS]/  />`, m`<WordCharacterSet /\\[wW]/  />`]));
  }
  *AnyCharacterSet() {
    yield eat(m`sigilToken: <*Keyword '.' />`);
  }
